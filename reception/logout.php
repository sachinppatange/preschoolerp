<?php
/**
 * reception/logout.php
 *
 * Reception area logout handler.
 *
 * Behavior:
 *  - Destroys the PHP session and clears the session cookie.
 *  - Removes common persistent-login cookies (e.g. 'remember_me', 'reception_remember').
 *  - Optionally revokes server-side tokens if your app stores them (example commented).
 *  - Returns JSON { ok: true, redirect: "<url>" } for AJAX requests or redirects to the reception login page for normal requests.
 ...
 *
 * Place at: /pioneerplayschool01/reception/logout.php
 */
...
if (!function_exists('safe_redirect_url')) {
     ...
     */
    function safe_redirect_url(string $url, string $fallback = '/reception/login.php'): string {
        $url = trim($url);
...
$redirect_after = safe_redirect_url((string)$requested, function_exists('site_url') ? site_url('/reception/login.php') : '/reception/login.php');

/* Capture user identifier for optional logging/revocation */
$reception_user = auth_user();

/* Optional: revoke server-side tokens for this user (if your app stores sessions / remember tokens)
   Example (adapt to your schema):
if (!empty($reception_user) && function_exists('db_run')) {
    try {
        db_run("DELETE FROM user_tokens WHERE user_identifier = ?", [is_array($reception_user) ? ($reception_user['id'] ?? $reception_user) : $reception_user]);
    } catch (Throwable $e) {
        error_log('Failed to revoke tokens for reception user: ' . $e->getMessage());
    }
}
*/
...
if (!empty($reception_user) && function_exists('db_run')) {
    try {
        ...
        // db_run("INSERT INTO audit_logs (user_identifier, event, ip, created_at) VALUES (?, 'reception_logout', ?, NOW())", [$reception_user, $_SERVER['REMOTE_ADDR'] ?? '']);
    } catch (Throwable $e) {
